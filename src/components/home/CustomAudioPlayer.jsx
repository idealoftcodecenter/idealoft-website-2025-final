...
export default function CustomAudioPlayer({ src }) {
    ...
    const handleSeek = (e) => {
        ...
        const percentage = x / boundingBox.width;
    ...
    return (
        <div className="audio-player-wrapper w-full max-w-2xl flex items-center bg-white py-4">
            ...
            <div className="flex-1 relative">
                ...
                <div className="absolute top-0 left-0 h-full bg-transparent pointer-events-none" style={{ width: `${progress}%` }} ></div>
            </div>
            ...
            <button onClick={() => {
				...
				const shouldRepeat = !isRepeating;
				setIsRepeating(shouldRepeat);
				...
				if (shouldRepeat && isPlaying) {
					wavesurferRef.current.play();
				}
			}} className={`w-[40px] h-[40px] text-black ${ progress > 0 ? "opacity-100" : "opacity-50" }`} title="Toggle Repeat" >
