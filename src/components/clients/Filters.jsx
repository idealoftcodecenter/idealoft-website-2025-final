...
export default function Filters({ tabs, activeTab, onTabClick }) {
    ...
    return (
        <section className="w-full mt-[var(--spacing-unit)] border-y-2 border-y-[#E0E5F6] tab-list flex lg:overflow-hidden" style={{ height: "calc(var(--spacing-unit) + 1px)" }}>
			<ul className='hidden lg:flex items-center h-full relative mx-[-8px]'>
				{tabs.map((tab, index) => {
					...
					return (
						<li key={tab.type} onClick={(e) => onTabClick(index)} className='relative z-10 group overflow-hidden skew-x-[-8deg] border-r-2 border-r-[#E0E5F6]'>
							<button className={`cursor-pointer relative flex items-center justify-center transition-all duration-300 group px-6 xl:px-12 skew-x-[8deg] ${isActive && 'active text-white font-bold'}`}>
								...
								<span className="pl-5 uppercase lg:text-[14px] xl:text-[16px]">{tab.label}</span>
							</button>
						</li>
					);
				})}
			...
			<p className='lg:hidden h-full flex items-center justify-center w-1/3 border-r-2 border-r-[#E0E5F6] bg-white'>Filter</p>
