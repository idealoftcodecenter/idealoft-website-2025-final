		...
		<div className="flex flex-col xl:flex-row xl:items-stretch w-full" style={{ height: isMounted && isMobile ? "calc(var(--spacing-unit) * 25)" : "calc(var(--spacing-unit) * 12)" }}>
			<div key={`tab-fix-content-${tabData?.name}`} className="flex-1 bg-white border-x border-x-[#E0E5F6] border-b border-b-[#E0E5F6] relative">
				<div className="px-8 flex flex-col space-y-10">
					<div className="max-w-[520px]" style={{ marginTop: "var(--spacing-unit)" }}>
						{
							...
							leftTabData.paragraphs.map((para, key) => <p key={key} className={`leading-[1.4em] text-md md:text-lg lg:text-xl mb-6`} style={{ height: "auto" }}>{parse(para.text)}</p>)
						}
					</div>

					<div className={`flex items-center justify-between gap-4 md:gap-10 md:mt-[calc(-1 * var(--spacing-unit))]`}>
						{
							leftTabData.logos && leftTabData.logos.length > 0 && 
							leftTabData.logos.map((logo, key) => {
								return (
									<div key={key} className={`flex-1 flex items-center justify-center`} style={{ height: "calc(var(--spacing-unit) * 4) "}}>
					...
					<p className="hidden md:block text-center font-normal text-[1.4em] text-[#E0E5F6] font-sans" style={{ height: "var(--spacing-unit)", marginTop: "calc(-0.5 * var(--spacing-unit))", marginBottom: "calc(0.5 * var(--spacing-unit))" }}>+ Many More</p>
				</div>
				...
				<div className="pl-10 pr-6 max-sm:pl-8 max-sm:pr-8" style={{ marginTop: "var(--spacing-unit)" }}>
					<div className="block lg:flex lg:items-start lg:space-x-[18px]">
						<div className="flex w-full">
							<div className="flex-none shrink-0 w-[42px] h-[42px] lg:w-[80px] lg:h-[80px] rounded-full overflow-hidden border bg-[#FEE5E5] text-[#DE90A5] grid place-items-center mb-2">
								{tabData?.profilePic ? (
									<Image
										src={tabData.profilePic}
										width={80}
										height={80}
										alt={`Idealoft client testimonial from ${tabData?.name}`}
										className="w-full h-full object-cover"
									/>
								) : (
									<span className="font-display text-[20px] lg:text-[28px] font-bold">{initials}</span>
								)}
							</div>
							...
							<div className="flex-1 min-w-0 pl-4 lg:pl-6">
								...
								<span className="block text-gray-200 font-sans font-bold italic text-md max-sm:text-[12px] leading-[1.2em]">
									<span className="text-[#B6BAC7] font-normal leading-[1.2em]">
										{tabData?.designation}
									</span>
									<br />
									<span className="text-cedar font-bold leading-[1.2em]">
										{tabData?.company}
									</span>
								</span>
							</div>
